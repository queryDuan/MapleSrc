...
public class MapleServerHandler extends IoHandlerAdapter {
    ...
    public void sessionOpened(IoSession session) throws Exception {
        ...
        if (!(LoginServer.getInstance().ipCanConnect(RemoteIP))) {
            session.close();
            return;
        //} else if (LocalIP.equals(LoginServer.Host)) {
            //if ((LoginServer.getInstance().hascheckIP(RemoteIP))) {
               // log.info("[{}]  找不到地址.关闭. ", RemoteIP);
               // session.close();
               // return;
           // }
            //log.info("[{}]  连接登录服务器.", RemoteIP);
        }
        LoginServer.getInstance().addConnectedIP(RemoteIP);
