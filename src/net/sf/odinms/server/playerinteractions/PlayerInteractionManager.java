        ...
        for (MaplePlayerShopItem pItems : items) {
            if (pItems.getBundles() > 0) {
                if (pItems.getItem().getType() == 1) {
                    ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO hiredmerchant (ownerid, itemid, quantity, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, owner,expiredate, vicious, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    ...
                    ps.setTimestamp(22, eq.getExpiration());
                    ps.setShort(23, eq.getVicious());
                    ps.setInt(24, 1);
                } else {
    ...
    public void tempItems(boolean overwrite) throws SQLException {
        ...
        if (overwrite) {
            for (MaplePlayerShopItem pItems : items) {
                if (pItems.getBundles() > 0) {
                    if (pItems.getItem().getType() == 1) {
                        ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO hiredmerchanttemp (ownerid, itemid, quantity, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, owner, expiredate, vicious, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                        Equip eq = (Equip) pItems.getItem();
                        ps.setInt(2, eq.getItemId());
                        ps.setInt(3, 1);
                        ps.setInt(4, eq.getUpgradeSlots());
