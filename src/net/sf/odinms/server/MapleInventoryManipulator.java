...
public class MapleInventoryManipulator {
    ...
    public static void equip(MapleClient c, byte src, byte dst) {
        ...
        if (source == null) {
         ...
         /* if (c.getPlayer().getReborns() <= 10 && c.getPlayer().getReborns() >=1) {//筑基期
            switch (source.getItemId()) {
                case 1302084: //隐屏喇叭不能给普通玩家使用
                 //如果需要增加其他的就 添加 case 代码;  即可
                    removeAllById(c, source.getItemId(), false);
                    c.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(1, "这件装备只有筑基期修士才能带!加把劲修炼去吧."));
                    return;
            }
        }*/
                  /*if (c.getPlayer().getReborns() <= 20 && c.getPlayer().getReborns() >=11) {//开光期
            switch (source.getItemId()) {
                case 1302084: //隐屏喇叭不能给普通玩家使用
                 //如果需要增加其他的就 添加 case 代码;  即可
                    removeAllById(c, source.getItemId(), false);
                    c.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(1, "这件装备只有开光期修士才能带!加把劲修炼去吧."));
                    return;
            }
        }*/
        if (!c.getPlayer().isGM()) {
            switch (source.getItemId()) {
                case 5370001: // Wizet Invincible Hat
                case 5370000: // Wizet Plain Suit
              //  case 1062007: // Wizet Plain Suit Pants
              //  case 1322013: // Wizet Secret Agent Suitcase
                    removeAllById(c, source.getItemId(), false);
                    c.getPlayer().dropMessage(1, "无法使用");
                    return;
            }
        }
        ...
        if (dst == -128) {
        c.getPlayer().dropMessage(1, "此位置不可以放置该物品");
        c.getSession().write(MaplePacketCreator.enableActions());
        return;
        }
        if (dst < -99) {
