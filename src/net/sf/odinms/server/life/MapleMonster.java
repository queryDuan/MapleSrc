...
public class MapleMonster extends AbstractLoadedMapleLife {
   ...
   public void giveExpToCharacter(MapleCharacter attacker, int exp, boolean highestDamage, int numExpSharers) {
        ...
        if (attacker.getHp() > 0) {
            ...
            if(attacker.getVip() >= 5){
                attacker.gainExp((int) (personalExp * 2), true, false, highestDamage);
                } else  if(attacker.getMapId() == 910000004 && attacker.getVip() >= 2){//VIP刷怪地图经验
                 attacker.gainExp((int) (personalExp*2), true, false, highestDamage);
                } else  if(attacker.getMapId() == 910000005 && attacker.getVip() >= 3){
                 attacker.gainExp((int) (personalExp*3), true, false, highestDamage);
                } else  if(attacker.getMapId() == 910000006 && attacker.getVip() >= 4){
                 attacker.gainExp((int) (personalExp*4), true, false, highestDamage);
                } else {
                    attacker.gainExp((int) (personalExp), true, false, highestDamage);
                }

//attacker.gainExp(personalExp*3, true, false, highestDamage);
//else
            /* if (attacker.getClient().getChannel() == ChannelServer.getInstance(attacker.getClient().getChannel()).getVipCh()) {
attacker.gainExp((int) (personalExp * ChannelServer.getInstance(attacker.getClient().getChannel()).getVipChExpRate()), true, false, highestDamage);
                } else {
